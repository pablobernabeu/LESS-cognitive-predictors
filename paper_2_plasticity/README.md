# Paper 2 — Cognitive Predictors and Neuroplasticity in Intensive Multilingual Learning
...
(Part A), plus pre/post change in cognition and resting-state activity across
the training period (Part B). Reuses the shared infrastructure in
...
|---|---|---|---|
| config | `scripts/_config.R` | — | sessions (cognitive S1/5, neural S2/6), EF task definitions, model-ID naming |
...
| 6 | `scripts/06_summaries.R` | **HPC** | pooled convergence/posterior-summary CSVs, end-of-training attainment, by-participant rate/attainment reliability |
...
| 10 | `scripts/10_extract_descriptives.R` | local | observed descriptives the manuscript injects and the descriptive figures read: cognitive indices, predictor values and correlations, resting-state band power and PSD, and the grammaticality-judgement accuracy trajectory |
...
`scripts/09_compare_aperiodic.R` and `hpc/10_compare_aperiodic.slurm` are kept for
reference and must not be submitted. That earlier comparison put the two models on
different samples (56 against 50 participants), which `loo_compare()` rejects, and the
script writes to the same output file as `09b`, so running it would overwrite the
artefact the manuscript reads.
...
leakage. That was a real bug, caught and fixed on 2026-07-02. (2) The selection uses the latent projection
...
`lme4::lmer`, avoiding the crash entirely, and is the projpred authors'
recommended projection for multilevel binomial models. The Bayesian (Stan) reference
...
recipe as Paper 1 (see its README).
...
`R/4.5.1-gfbf-2025a`, points `R_LIBS_USER` at the project library
`/data/educ-intract/educ1242/new_LESS/Rlib/R-4.5`, and pins `CXXFLAGS_OPTIM = -O1` in
CmdStan's `make/local` to avoid a GCC 14.2.0 crash on Stan's `reduce_sum` templates. The
per-stage recipes are in [`hpc/README.md`](hpc/README.md) and the layout in
[`../HPC_RUNBOOK.md`](../HPC_RUNBOOK.md). Rendering happens on the Windows dev box under a
different R and a different Quarto, both recorded in
[`../paper_1_transfer/README.md`](../paper_1_transfer/README.md). Versions are not restated
here. `results/_provenance.csv`, written by
[`../_shared/R/04_provenance.R`](../_shared/R/04_provenance.R) whenever
`scripts/04_fit_brms_predictors.R` runs, records the R, brms, cmdstanr, rstan, StanHeaders,
posterior, loo, projpred, bayesplot and CmdStan versions of that run together with the
seed. The R side of the same environment is pinned in the repository's `renv.lock`. Until that
script has run against the current results the file is absent, and the manuscript prints
`pending` in place of each version, as its Data and code availability section explains.
